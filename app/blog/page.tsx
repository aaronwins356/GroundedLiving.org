...
export default async function BlogIndexPage({ searchParams }: BlogIndexPageProps) {
  ...
  const searchValue = getSearchValue(searchParams?.search).toLowerCase();
  const pageValue = getSearchValue(searchParams?.page);
  ...
  const currentPage = Math.min(
    totalPages,
    Math.max(1, Number.parseInt(pageValue || "1", 10) || 1),
  );
        ...
        <div className="mx-auto flex max-w-4xl flex-col gap-8 text-center">
          ...
          <form className="mx-auto flex w-full max-w-xl gap-3" method="get">
            ...
            <input
              id="search"
              name="search"
              defaultValue={getSearchValue(searchParams?.search)}
